...
import { Activity, Database, TrendingDown, Zap, Gauge, ArrowRight, DollarSign } from "lucide-react";
...
export default async function HomePage() {
  ...
  return (
    <>
      ...
      {/* "What will it cost?" — replaces the old status bar, styled like the optimizer CTA */}
        ...
        <div className="mx-auto flex max-w-7xl flex-col items-center gap-4 px-4 py-8 sm:flex-row sm:px-6 lg:px-8">
          <div className="flex items-center gap-4">
            <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-ink-900">
              <DollarSign className="h-6 w-6 text-spotlight-400" />
            </div>
            <div>
              <h2 className="text-lg font-bold text-ink-900">What will it cost?</h2>
              <p className="text-sm text-gray-600">
                {status.recentPriceChanges} models updated recently
                {status.lastRun && <> · Last sync: {formatDateTime(status.lastRun)}</>}
                {" · "}
                <span className="font-medium text-gray-700">{status.totalModels} models · {status.totalProviders} providers</span>
              </p>
            </div>
          </div>
      ...
      {/* "Already using a model?" — optimizer CTA */}
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="mb-8 flex items-end justify-between">
          <div>
            ...
            <p className="mt-1 text-gray-500">Side-by-side analysis of leading models.</p>
          </div>
          <Link
            href="/compare"
          ...
          <Link href="/compare?models=gpt-5-6-sol,claude-opus-4-8,gemini-3-1-pro" className="group">
            <Card className="card-hover-lift h-full overflow-hidden">
              <CardContent className="p-6">
                ...
                <h3 className="text-lg font-semibold text-ink-900">GPT-5.6 vs Claude 4.8 vs Gemini 3.1</h3>
                <p className="mt-1 text-sm text-gray-500">Frontier model showdown</p>
                <p className="mt-4 text-sm font-semibold text-brand-600 opacity-0 transition-opacity group-hover:opacity-100">Compare now →</p>
              </CardContent>
            </Card>
          </Link>
          ...
          <Link href="/compare?models=gpt-5-4-mini,claude-4-5-haiku,gemini-3-1-flash-lite" className="group">
            <Card className="card-hover-lift h-full overflow-hidden">
              <CardContent className="p-6">
                ...
                <h3 className="text-lg font-semibold text-ink-900">Fast & Cheap Models</h3>
          ...
          <Link href="/compare?models=gpt-5-6-sol,deepseek-v4-pro,glm-5-2" className="group">
            <Card className="card-hover-lift h-full overflow-hidden">
              <CardContent className="p-6">
                ...
                <h3 className="text-lg font-semibold text-ink-900">Premium vs Open Source</h3>
