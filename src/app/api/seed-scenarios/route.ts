...
export async function POST() {
  try {
    ...
    return NextResponse.json({ success: true, inserted, updated, total: scenarios.length });
  } catch (error) {
    console.error("[api/seed-scenarios] Error:", error);
    return NextResponse.json({ error: "Failed to seed scenarios" }, { status: 500 });
  }
}
